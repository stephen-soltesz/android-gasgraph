 ...
 * 
 */
public class ViewFlow extends AdapterView<BaseAdapter> {
	...
	public ViewFlow(Context context, AttributeSet attrs) {
		...
		mSideBuffer = styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, 5);
		init();
	}

	private void init() {
		mLoadedViews = new LinkedList<View>();
		//mScroller = new Scroller(getContext(), new DecelerateInterpolator((float) 15.0));
		mScroller = new Scroller(getContext());
		//mScroller = new Scroller(getContext(), new OvershootInterpolator((float) 1.0));
		//mScroller = new Scroller(getContext(), new OvershootInterpolator((float) 1.0));
		final ViewConfiguration configuration = ViewConfiguration
				.get(getContext());
		mTouchSlop = configuration.getScaledTouchSlop()*2;
		//mTouchSlop = configuration.getScaledTouchSlop();
		Lg.d("slop: " + mTouchSlop);
		mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();
		Lg.d("velocity: " + mMaximumVelocity);
	}
	...
	public int getViewsCount() {
		if ( mAdapter == null ) { Log.e("err", "madapter == null"); }
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				Lg.e("velX: " + velocityX + " snap: " + SNAP_VELOCITY + " cur: " + mCurrentScreen);
				if (velocityX > SNAP_VELOCITY && mCurrentScreen > 0) {
					// Fling hard enough to move left
					snapToScreen(mCurrentScreen - 1);
				} else if (velocityX < -SNAP_VELOCITY
	...
	public void setFixed(boolean fixedOn) {
		mFixed = fixedOn;
	}
	
	public void setAdapter(BaseAdapter adapter, int initialPosition) {
		if (mAdapter != null) {
	...
	private void resetFocus() {
		...
		for (int i = Math.max(0, mCurrentAdapterIndex - mSideBuffer); 
			 i < Math.min(mAdapter.getCount(), mCurrentAdapterIndex + mSideBuffer + 1); 
			 i++) 
		{
