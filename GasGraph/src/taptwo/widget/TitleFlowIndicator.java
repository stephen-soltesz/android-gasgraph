 ...
 */
package taptwo.widget;

import java.util.ArrayList;

import taptwo.widget.FlowIndicator;
...
public class TitleFlowIndicator extends TextView implements FlowIndicator {
	...
	private static final float TITLE_PADDING = 20.0f;
	...
	public TitleFlowIndicator(Context context, AttributeSet attrs) {
		...
		//TypedArray styledAttrs = context.obtainStyledAttributes(attrs, R.styleable.TitleFlowIndicator);
		...
		footerColor = a.getColor(R.styleable.TitleFlowIndicator_footerColor, FOOTER_COLOR);
		...
		//boolean selectedBold = true; // a.getBoolean(R.styleable.TitleFlowIndicator_selectedBold, SELECTED_BOLD);
		boolean selectedBold = false; // a.getBoolean(R.styleable.TitleFlowIndicator_selectedBold, SELECTED_BOLD);
		...
		Lg.d("tf footerheight:   " + footerLineHeight + "\n" +
		     "tf footertriangle: " + footerTriangleHeight + "\n" +
		     "tf bold: " + selectedBold + "\n" +
		     "tf color: " + selectedColor + "\n" +
		     "tf size: " + textSize + "\n" +
		     "tf selected: " + selectedSize + "\n" +
		     "tf padding: " + titlePadding + "\n" +
		     "tf clip: " + clipPadding);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			...
			if ((bound.left > getLeft() && bound.left < getLeft() + getWidth()) || (bound.right > getLeft() && bound.right < getLeft() + getWidth())) {
				...
				//canvas.drawText(title, bound.left, bound.bottom-footerTriangleHeight/2, paint);
		...
		//path.moveTo(0, coordY+1);
		//path.lineTo(getWidth(), coordY+1);
		//path.close();
		//canvas.drawPath(path, paintFooterLine);
		// Draw the footer triangle
		path = new Path();
		path.moveTo(getWidth() / 2, getHeight() - footerTriangleHeight-9);
		path.lineTo(getWidth() / 2 + footerTriangleHeight, getHeight() - 9);
		path.lineTo(getWidth() / 2 - footerTriangleHeight, getHeight() - 9);
		path.close();
		canvas.drawPath(path, paintFooterTriangle);
		
		GradientDrawable gradient = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] { footerColor, 0});
		gradient.setGradientType(GradientDrawable.LINEAR_GRADIENT);
		//gradient.setGradientRadius(canvas.getWidth() * 2);
		gradient.setDither(true);
		gradient.setGradientCenter(0,0);
		gradient.setBounds(0, getHeight()-9, getWidth(), getHeight());
